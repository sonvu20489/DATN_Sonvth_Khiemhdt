...
void Finger_changename(const char * name, int id)
{
  lcd.clear();
  PrintL("Change name Finger..",1,0);
  if((id>20) || (id<1))
  {
    PrintL("ID is not exist",2,0);
    delay(1000);
    return ;
  }
  if((EEPROM.read(ADD_MAP_FINGER+(id-1))!=100))
  ...
  strcpy(Finger_name[id-1],name);
  for(int i = 0; i<10;i++)
  {
    EEPROM.write((ADD_F_NAME_STORE+((id-1)*10)+i),Finger_name[id-1][i]);
    EEPROM.commit();
  }
  lcd.setCursor(0, 2);
