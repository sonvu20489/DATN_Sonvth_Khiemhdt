...
void init_RFID()
{
  ...
  if(EEPROM.read(ADD_RFID_NSTORE)==0)
  {
    Serial.println("{\"DelRFID\":\"1\"}");
    return ;
  }
  ...
  for(int i = 0; i<MAXRFID;i++)
  {
    if(EEPROM.read(ADD_UID_CARD + 4*i) == 0)
    {
      continue;
    }
    if(strcmp(RFID_name[i],"")==0)
    {
      continue;
    }
    Name.add(RFID_name[i]); //save name
    pos.add(i+1);
    JsonArray RFID_arr = RFID.createNestedArray();
    for(int j = 0;j<4;j++)
    {
      RFID_arr.add(EEPROM.read(ADD_UID_CARD + 4*i + j));
    }
  }
...
void writeID(byte a[]){
  ...
  if(!findID(a)){// Before we write to the EEPROM, check to see if we have seen this card before!
    ...
    doc["type"] = "RFID";
    doc["action"] = "add";
    doc["id"] = String(slot_add + 1);
    JsonArray RFID_code = doc.createNestedArray("RFID_code");
    for(int i = 0;i<4;i++)
    {
      RFID_code.add(a[i]);
...
void deleteID(byte a[]){
  if(!findID(a)){ // Before we delete from the EEPROM, check to see if we have this card!
    ...
    doc["type"] = "RFID";
    doc["action"] = "del";
    doc["id"] = String(slot+1);
    doc["name"] = RFID_name[slot];
    JsonArray RFID_code = doc.createNestedArray("RFID_code");
    for(int i = 0;i<4;i++)
    {
      RFID_code.add(a[i]);
...
void RFID_changename(const char * name, int slot)
{
  lcd.clear();
  PrintL(" Change name RFID...",1,0);
  slot = slot-1;
  if(slot >= 20)
  {
    PrintL("Slot is not exist ",2,0);
    delay(1000);
    return ;
  }
  uint8_t Read[4];
  ...
  strcpy(RFID_name[slot], name);
  for(int i = 0;i<10;i++)
  {
    EEPROM.write((slot*10)+ADD_NAME_STORE+i,RFID_name[slot][i]);
    EEPROM.commit();
  }
  lcd.setCursor(0, 2);
...
void handle_RFID()
{
  ...
  if(successRead)
  {
    if(findID(readCard))
    {
      ...
      accgranted((const char*)RFID_name[slot]);
      lcd.clear();
