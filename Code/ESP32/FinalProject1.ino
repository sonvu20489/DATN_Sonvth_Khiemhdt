...
void setup() {
  ...
  Serial.println(EEPROM.read(520));
  init_LCD();
  init_keypadcheck();
...
void ReadJson()
{
  while(Serial.available())
  {
    ...
    else
    {
      ...
      if(json["Del_RFID"])
      {
        int ID_DEL = json["Del_RFID"];
        deleteRFID_ID(ID_DEL);
        MenuDisplay(menu);
      }
      if(json["Del_FingerID"])
      {
        int ID_DEL = json["Del_FingerID"];
        deleteFingerprint(ID_DEL);
        MenuDisplay(menu);
      }
      if(json["name"] && json["ch_RFID"])
      {
        const char * name = json["name"];
        int ch_id = json["ch_RFID"];
        RFID_changename(name,ch_id);
        MenuDisplay(menu);
      }
      if(json["name"]&& json["ch_FingerID"])
      {
        const char * name = json["name"];
        int ch_id = json["ch_FingerID"];
        Finger_changename(name,ch_id);
        MenuDisplay(menu);
      }
    }
  }
